...
static dd4hep::Ref_t createTkLayoutTrackerEndcap(dd4hep::Detector& lcdd,
  ...
  if (xmlDet.hasAttr(_U(region))) worldDetElement.setRegion(lcdd, xmlDet.regionStr(), bothEndcaps);
  // set limits - if given
  if (xmlDet.hasAttr(_U(limits))) worldDetElement.setLimitSet(lcdd, xmlDet.limitsStr(), bothEndcaps);
