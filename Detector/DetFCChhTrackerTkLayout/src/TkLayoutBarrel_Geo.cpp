...
static dd4hep::Ref_t createTkLayoutTrackerBarrel(dd4hep::Detector& lcdd,
  ...
  if (xmlDet.hasAttr(_U(region))) topDetElement.setRegion(lcdd, xmlDet.regionStr(), motherVol);
  // set limits - if given
  if (xmlDet.hasAttr(_U(limits))) topDetElement.setLimitSet(lcdd, xmlDet.limitsStr(), motherVol);
